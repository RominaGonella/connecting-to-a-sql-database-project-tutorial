...
(2,'A Book Apart'), (3,'A K PETERS'), (4,'Academic Press'), (5,'Addison Wesley'),
(6,'Albert&Sweigart'), (7,'Alfred A. Knopf');
""")

# authors #
engine.execute("""
INSERT INTO authors (author_id, first_name, middle_name, last_name) VALUES (1,'Merritt',null,'Eric'),
(2,'Linda',null,'Mui'), (3,'Alecos',null,'Papadatos'), (4,'Anthony',null,'Molinaro'), (5,'David',null,'Cronin'),
(6,'Richard',null,'Blum'), (7,'Yuval','Noah','Harari'), (8,'Paul',null,'Albitz');
""")
...
# book authors: dejé solo un registro porque estaba duplicado 4 veces y daba error #
engine.execute("""
...
tab1 = pd.read_sql("SELECT * FROM publishers", engine)
print('La tabla publishers tiene ' + str(len(tab1)) + ' registros y ' + str(len(tab1.columns)) + ' variables')
tab2 = pd.read_sql("SELECT * FROM authors", engine)
print('La tabla authors tiene ' + str(len(tab2)) + ' registros y ' + str(len(tab2.columns)) + ' variables')
tab3 = pd.read_sql("SELECT * FROM books", engine)
print('La tabla books tiene ' + str(len(tab3)) + ' registros y ' + str(len(tab3.columns)) + ' variables')
tab4 = pd.read_sql("SELECT * FROM book_authors", engine)
print('La tabla book_authors tiene ' + str(len(tab4)) + ' registros y ' + str(len(tab4.columns)) + ' variables')
print()
print("Tabla authors:")
print(tab2)